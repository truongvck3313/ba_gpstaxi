...
class IconPage:
    ...
    async def hidden_car_lis_history(self, code, event, result):
        ...
        try:
            ...
            if (stt == "1") and (liscens_plate != "") and (status != "") and (person != "") and (day_action != ""):
                logging.info("Pass")
                module_other_gpstaxi.writeData(checklistpath, "Checklist", code, 7, "Pass")
            else:
                logging.info("Fail")
...
class ListVehicle:
    ...
    async def System_status_excel(self, code, event, result):
        ...
        await module_other_gpstaxi.write_result_web_excel_dowload(self.page, code, event, result, "Hiện trạng hệ thống - Xuất excel",
                                                      var_gpstaxi.btnExportExcel, "HienTrangHeThong.xlsx")
